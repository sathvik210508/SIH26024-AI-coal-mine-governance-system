...
import { CheckSquare, Upload, Camera, Clock, AlertCircle, CheckCircle2, BrainCircuit, GitCommit, MapPin, ShieldCheck, ExternalLink } from "lucide-react";
...
export const FieldCorrectiveActions: React.FC = () => {
  ...
  const [coords, setCoords] = useState<{ lat: number; lng: number }>({ lat: 24.1988, lng: 82.6651 });
  ...
  useEffect(() => {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => setCoords({ lat: parseFloat(pos.coords.latitude.toFixed(4)), lng: parseFloat(pos.coords.longitude.toFixed(4)) }),
        () => setCoords({ lat: 24.1988, lng: 82.6651 })
      );
    }
  }, []);
  ...
  const handleSubmitEvidence = async (e: React.FormEvent) => {
    ...
    setUploading(true);
    try {
      const res = await api.patch(`/field/corrective-actions/${selectedAction.id}`, {
      ...
      if (res.data?.ai_verification) {
        setAiVerificationResult({
          action_id: selectedAction.action_id,
          ...res.data.ai_verification,
        });
      }
      setSuccessMsg(
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {/* AI Verification Real-Time Feedback Banner */}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <span className="text-xs font-bold text-slate-900">
                  AI Automated Verification Result — {aiVerificationResult.action_id}
                </span>
                <span className="text-[11px] text-slate-600 block">
                  Ground proof cryptographically anchored to SHA-256 audit chain
                </span>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded bg-emerald-100 border border-emerald-300 text-emerald-800 font-mono text-xs font-bold">
                {aiVerificationResult.status === "PASSED" ? "VERIFICATION PASSED" : "FLAGGED FOR REVIEW"} • {aiVerificationResult.confidence_score}%
              </span>
              <button
                onClick={() => setAiVerificationResult(null)}
                className="text-xs font-mono text-slate-400 hover:text-slate-600 ml-2"
              >
                ✕
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs font-mono">
            <div className="p-2 bg-white rounded border border-slate-200">
              ...
              <span className="text-emerald-700 font-semibold flex items-center gap-1 mt-0.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> {aiVerificationResult.gps_check}
              </span>
            </div>
            <div className="p-2 bg-white rounded border border-slate-200">
              <span className="text-slate-500 block text-[10px]">SHIFT CURRENCY</span>
              <span className="text-emerald-700 font-semibold flex items-center gap-1 mt-0.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> {aiVerificationResult.timestamp_check}
              </span>
            </div>
            <div className="p-2 bg-white rounded border border-slate-200">
              <span className="text-slate-500 block text-[10px]">VISUAL REMEDIATION</span>
              <span className="text-emerald-700 font-semibold flex items-center gap-1 mt-0.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> {aiVerificationResult.visual_markers_check}
              </span>
            </div>
          </div>
          <p className="text-[11px] text-slate-600 italic">
            Note: Action status is now AWAITING_VERIFICATION. Mine Manager review is required before statutory closure.
          </p>
        </div>
      )}
        ...
        {actions.map((act) => (
          ...
          >
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
              <button
                onClick={() => setLifecycleModal({ isOpen: true, actionId: act.id })}
                className="px-2.5 py-1.5 rounded bg-slate-100 hover:bg-slate-200 text-slate-700 font-mono text-xs flex items-center gap-1 transition-colors border border-slate-200"
                title="View Full 10-Stage Audit Trail & Merkle Hash"
              >
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-white border border-slate-200 rounded-xl p-5 shadow-2xl space-y-4">
            ...
            <form onSubmit={handleSubmitEvidence} className="space-y-3 text-xs">
              ...
              <div className="p-2.5 rounded bg-slate-100 border border-slate-200 text-[11px] text-slate-700 flex items-center justify-between font-mono">
                <span className="flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 text-slate-500" />
                  <span>Geo-Coordinates: {coords.lat}° N, {coords.lng}° E</span>
                </span>
                <span className="text-emerald-700 font-semibold">GPS Matched</span>
              </div>
